...
export default function AgentsOverview({ onConversations, onCase }) {
      ...
      {!data ? (
        ...
        <>
          ...
          <div className="portfolio-grid agents-grid">
            {[0, 1].map((column) => (
              <div className="agents-column" key={column}>
                {data.agents.map(
                  (a, index) =>
                    index % 2 === column && (
                      <article
                        className="card portfolio-card agent-card"
                        key={a.id}
                        style={{ order: index }}
                      >
                        ...
                        <p>
                          ...
                          {a.kind === 'retrieval' ? 'Retrieval specialist' : 'AI agent'}
                        </p>
                        <p>{a.description}</p>
                            ...
                            <dl className="agent-card-stats" aria-label={`${a.name} work status`}>
                              {['running', 'queued', 'failed', 'completed'].map((status) => (
                                <div
                                  key={status}
                                  className={
                                    status === 'failed' && a.stats[status] ? 'has-failures' : ''
                                  }
                                >
                                  <dt>{status}</dt>
                                  <dd>
                                    {new Intl.NumberFormat('en-GB').format(a.stats[status] || 0)}
                                  </dd>
                                </div>
                              ))}
                            </dl>
